...
function EmailSendScreen() {
  ...
  const handleSelectionChange = (emails) => {
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
        ...
        <TouchableOpacity 
          style={[
            styles.sendButton, 
            (selectedEmails.length === 0 || (!includePlanning && !includeStock && !includeShoppingList) || loading) && 
            styles.sendButtonDisabled
          ]} 
          onPress={handleSendEmail}
          disabled={loading || selectedEmails.length === 0 || (!includePlanning && !includeStock && !includeShoppingList)}
        >
          {loading ? (
